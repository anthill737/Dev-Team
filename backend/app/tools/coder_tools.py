    ...
    return [
        ...
        ToolSpec(
            ...
            description=(
                ...
                "you're done.\n\n"
                "  status='approved' — acceptance criteria met AND you verified this "
                "with something a bash command can check (tests pass, file has expected "
                "contents, a Node script confirmed the behavior). Never mark approved "
                "based on 'the code looks right' alone. Provide `summary`.\n\n"
                "  status='needs_user_review' — acceptance criteria involve something "
                "you CAN'T verify from bash: how a UI looks, whether a game feels right, "
                "whether output is visually correct, whether a browser renders something. "
                "Required when any acceptance criterion mentions 'browser', 'visible', "
                "'displays', 'looks', or similar. Provide `summary`, `review_checklist` "
                "(steps the user should take to verify — be specific, e.g. 'Open "
                "index.html in a browser' is better than 'check the UI'), and "
                "`review_run_command` (exact shell command the user can copy and paste "
                "to run or view what you built — no guesswork on their part). "
                "Optionally `review_files_to_check` listing key files.\n\n"
