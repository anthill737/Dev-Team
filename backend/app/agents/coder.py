...
class Coder:
    ...
    async def _stream_inner(
        ...
        *,
        ...
        # Read platform + playwright_enabled fresh from disk each run so a
        # mid-project toggle (user edits via PATCH) takes effect on the next
        # iteration. playwright_enabled changes whether the Coder routes UI
        # tasks to needs_user_review (off) or approved (on, Reviewer verifies).
        meta = ctx.store.read_meta()
